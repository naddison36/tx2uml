...
describe("Ethereum Node Clients", () => {
    const clients: { [clientName: string]: EthereumNodeClient } = {
        Geth: new GethClient(process.env.GETH_URL, "ethereum"),
        rETH: new OpenEthereumClient(process.env.RETH_URL, "ethereum"),
    }
    ...
    describe.each(Object.entries(clients))("%s", (clientName, nodeClient) => {
        ...
        test.only("Get token details", async () => {
            ...
            expect(tokenDetails[2].tokenSymbol).toEqual("USDC")
            expect(tokenDetails[2].tokenName).toEqual("USD Coin")
            expect(tokenDetails[2].decimals).toEqual(6)
            expect(tokenDetails[3].tokenSymbol).toEqual("")
            expect(tokenDetails[3].tokenName).toEqual("")
            expect(tokenDetails[4].tokenSymbol).toEqual("mUSD")
            expect(tokenDetails[4].tokenName).toEqual("mStable USD")
            expect(tokenDetails[4].decimals).toEqual(18)
            expect(tokenDetails[5].tokenSymbol).toEqual("")
            expect(tokenDetails[5].tokenName).toEqual("")
