...
exports.streamSingleTxPuml = (pumlStream, transaction, contracts, options = {}) => {
...
exports.writeTransfers = (plantUmlStream, transfers, options = {}) => {
    ...
    for (const transfer of transfers) {
        debug(`id ${transfer.id}, from ${exports.shortAddress(transfer.from)}, to ${exports.shortAddress(transfer.to)}, value ${transfer.value.toString()}`);
        const valueUnit = transfer.symbol
            ? transfer.symbol
            : "ETH";
        plantUmlStream.push(`${exports.participantId(transfer.from)} -> ${exports.participantId(transfer.to)}: ${transfer.value.toString()} ${valueUnit}\n`);
    }
};
