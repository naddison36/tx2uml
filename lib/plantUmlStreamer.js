...
exports.writeMessages = (plantUmlStream, messages, options = {}) => {
    ...
    for (const message of messages) {
        ...
        if (message.type === transaction_1.MessageType.Call ||
            ...
            // output call message
            plantUmlStream.push(`${exports.participantId(message.from)} ${genArrow(message)} ${exports.participantId(message.to)}: ${genFunctionText(message, options.params)}${genGasUsage(message, options.gas)}\n`);
            ...
            plantUmlStream.push(`${exports.participantId(message.from)} ${genArrow(message)} ${exports.participantId(message.to)}: ${ethers.toFormat(2)} ETH${genGasUsage(message, options.gas)}\n`);
