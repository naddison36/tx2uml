...
const writeMessages = (plantUmlStream, traces, options = {}) => {
    ...
    for (const trace of traces) {
        ...
        if (trace.type === transaction_1.MessageType.Selfdestruct) {
            plantUmlStream.push(`${formatters_1.participantId(trace.from)} ${genArrow(trace)} ${formatters_1.participantId(trace.to)}: Self-Destruct\n`);
...
const genEndLifeline = (trace, options = {}) => {
    ...
    if (!trace.error) {
        ...
        plantUml += `note right of ${formatters_1.participantId(trace.to)}: ${trace.error}\n`;
    }
    return plantUml;
};
