...
const generateValueDiagram = async (hashes, options) => {
    ...
    const participants = await txManager.getTransferParticipants(transactionTransfers, transactions[0].blockNumber, options.chain, options.configFile);
    // Convert transactions and transfers to readable stream
    const pumlStream = (0, transfersPumlStreamer_1.transfers2PumlStream)(transactions, transactionTransfers, participants);
