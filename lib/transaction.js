...
class TransactionManager {
    ...
    async getContractsFromTraces(transactionsTraces, configFilename) {
    ...
    async getTransferParticipants(transactionsTransfers, block, configFilename) {
        ...
        const labels = (0, path_1.basename)(__dirname) === "lib"
            ? require("./labels.json")
        ...
        for (const token of tokenDetails) {
            ...
            if (!token.noContract &&
                ...
                const contract = await this.etherscanClient.getContract(implementation || address);
    ...
    async configOverrides(contracts, filename) {
