    ...
    async getContracts(transactionsTraces) {
        ...
        for (const trace of Object.values(flatTraces)) {
            ...
            if (trace.type === MessageType.DelegateCall) {
                ...
                !delegatesToContracts[trace.from].includes(trace.delegatedFrom)) {
        ...
        // map the delegatedFromContracts on each contract
        for (const [address, delegatedFromAddresses] of Object.entries(delegatesToContracts)) {
            contracts[address].delegatedToContracts =
                // map the delegatedTo addresses to Contract objects
                delegatedFromAddresses.map(delegateAddress => ({
                    ...contracts[delegateAddress],
                    address,
                }));
        }
