...
exports.getTokenTransfers = async (txHash, apiKey, network = "mainnet") => {
    ...
    try {
        ...
        if (!Array.isArray((_a = response === null || response === void 0 ? void 0 : response.data) === null || _a === void 0 ? void 0 : _a.data)) {
            throw new Error(`no token transfers in Alethio response ${response === null || response === void 0 ? void 0 : response.data}`);
        }
        for (const transfer of response.data.data) {
            let id = 1;
            let type = "TokenTransfer";
            if ((_b = transfer.relationships.contractMessage.data) === null || _b === void 0 ? void 0 : _b.id) {
                const idString = transfer.relationships.contractMessage.data.id.split(":")[2];
                id = parseInt(idString);
                type = convertType(transfer.relationships.contractMessage.data.type);
            }
            else if ((_c = transfer.attributes) === null || _c === void 0 ? void 0 : _c.globalRank[2]) {
            ...
            transfers.push({
                ...
                value: convertDecimal(transfer.attributes.value, transfer.attributes.decimals),
        ...
        throw new verror_1.VError(err, `Failed to get token transfers for transaction hash ${txHash} from Alethio at url ${url}`);
    }
};
exports.getEtherTransfers = async (txHash, apiKey, network = "mainnet") => {
    const contractMessages = await exports.getContractMessages(txHash, apiKey, network);
    const etherMessages = contractMessages.filter(message => {
        var _a;
        return (_a = message.value) === null || _a === void 0 ? void 0 : _a.gt(0);
    });
    debug(`Got ${etherMessages.length} Ether transfers from ${contractMessages.length} contract messages`);
    const [, firstMessage] = await exports.getTransactionDetails(txHash, apiKey, network);
...
exports.getTransfers = async (txHash, apiKey, network = "mainnet") => {
    const etherTransfers = await exports.getEtherTransfers(txHash, apiKey, network);
    const tokenTransfers = await exports.getTokenTransfers(txHash, apiKey, network);
