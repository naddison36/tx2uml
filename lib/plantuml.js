...
const genPumlJavaOptions = (options = {}) => {
    ...
    if (options?.format) {
        if (!tx2umlTypes_1.outputFormats.includes(options.format)) {
            throw new Error(`Invalid format ${options.format}. Must be either: ${JSON.stringify(tx2umlTypes_1.outputFormats)}`);
        }
