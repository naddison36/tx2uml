...
const genEndLifeline = (trace, options = {}) => {
    ...
    if (!trace.error) {
        if (options.noParams) {
            ...
            plantUml += `return${(0, exports.genParams)(trace.outputParams, options.noParamValues)}\n`;
        }
...
const genFunctionText = (trace, options) => {
    ...
    if (trace.type === tx2umlTypes_1.MessageType.Create) {
        ...
        if (trace.parsedConstructorParams) {
            return `${trace.funcName}(${(0, exports.genParams)(trace.inputParams, options.noParamValues)})`;
        }
    ...
    for (const contract of Object.values(contracts)) {
        if (contract.ethersContract &&
            ...
            for (const event of contract.events) {
                plantUmlStream.push(`\n${event.name}:`);
                plantUmlStream.push(`${(0, exports.genParams)(event.params, options.noParamValues).replace(/\\n/g, "\n  ")}`);
            }
            plantUmlStream.push("\nend note\n");
        }
    }
